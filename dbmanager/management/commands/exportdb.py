...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '-gz', '--compress', action='store_true',
            help='to compress dump file',
        )
        parser.add_argument(
            '-itbl', '--ignore-table', nargs='+',
            help='''To ignore specified table(s) which must be specified using both
                the database and table names. eg: dbname.tbl_name''',
        )
        parser.add_argument(
            '-tbl', '--tables', nargs='+',
            help='''To dump specified table(s) which must be specified after db name.
                eg: -d dbname -tbl tbl_name1, tbl_name2....''',
        )
        ...
        if db_keys:
            for db_key in db_keys:
                ...
                if engine == 'dummy':
                    ...
                    self.connector.ignore_tabled = options.get(
                        'ignore_table',
                    ) or []
            ...
            self.stdout.write(
                self.style.HTTP_INFO(
                    'No database(s) available to backup/restore.',
                ),
            )
    ...
    def dump_db(self, database):
        ...
        self.stdout.write(
            self.style.WARNING(
                'Selected Database: ' + database.get('NAME'),
            ),
        )
